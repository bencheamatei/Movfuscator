...
Implementarea noastră a movfuscatorului convertește în 'cod movfuscat' un fișier assembly primit ca input. În particular, acesta rescrie operațiile aritmetice și logice de bază (xor, and, or, not, add, sub, div, mul, inc, dec, shl, shr și în plus push și pop) folosind doar instrucțiunea mov (precum și operații de jump). Cum se realizează acest lucru? Ne folosim de mai multe lookup tables de $2^8 * 2^8$ care sunt generate în prealabil - acestea sunt generate ca fișiere binare. Dacă folosim doar aceste tabele putem ușor să implementăm aritmetica pe 8 biți. Cum extrapolăm la 32? Privim fiecare operație ca fiind 4 operații separate pe cele 4 blocuri de 8 biți. În cazul funcțiilor logice, acest lucru se realizează ușor, căci rezultatele de pe cele 4 blocuri sunt independente unele de altele. În cazul operațiilor aritmetice (e.g. add) trebuie să ținem cont de carry, iar în acest caz generăm un lookup table de 2 ori mai mare (prima parte fiind pentru adunarea fără carry, iar cealaltă pentru adunarea cu carry).
